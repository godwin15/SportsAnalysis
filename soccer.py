...
def fixturedf(rep):
    ...
    data = {
        "Home Team": [
            rep["response"][i]['teams']['home']['name'] for i in range(
                len(
                    rep["response"])) if i < 20], "Away Team": [
            rep["response"][i]['teams']['away']['name'] for i in range(
                len(
                    rep["response"])) if i < 20], "Date_and_Time": [
            rep['response'][i]['fixture']['date'] for i in range(
                len(
                    rep["response"])) if i < 20]}
...
def wins_draws(rep):
    ...
    for i in range(len(rep["response"])):
        if name_1 == rep["response"][i]['teams']['home']['name']:
            if rep['response'][i]['teams']['home']['winner']:  # counting the wins for name_1
                win_1 += 1
            if rep['response'][i]['teams']['home']['winner'] is None:  # counting the draws for both
                draws += 1
        if name_1 == rep["response"][i]['teams']['away']['name']:
            if rep['response'][i]['teams']['away']['winner']:  # counting the wins for name_1
                win_1 += 1
            if rep['response'][i]['teams']['away']['winner'] is None:  # counting the draws for both
                draws += 1
        if name_2 == rep["response"][i]['teams']['away']['name']:
            if rep['response'][i]['teams']['away']['winner']:  # counting the wins for name_2
                win_2 += 1

        if name_2 == rep["response"][i]['teams']['home']['name']:
            if rep['response'][i]['teams']['home']['winner']:  # counting the wins for name_2
                win_2 += 1
